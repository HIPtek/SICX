...
public class FECFile {
   ...
   //static Category cat = Category.getInstance(FECFile.class);
	...
	this.blockSize = params.getUnexpandedBlockSize();
        ...
        setDecodeExceptionHandler(new ExceptionHandler() {
                public void handleException(ExceptionEvent ev) {
                    log.warn("Set an ExceptionHandler via "+
	...
	if (mode.equals("rw")) {
            ...
            /* below removed by mikko.pitkanen@hip.fi */
            /* because we check integrity outside this package*/
            /*
            if (integrity == null) {
	...
	read(bufs,blockNum,stripeNums);
        ...
        /* below is also removed by mikko.pitkanen@hip.fi since we check the integrity ouside this code */
